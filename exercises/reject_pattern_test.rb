...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_words_containing_e
    ...
    selected = []
		words.each do |word|
			selected << word unless word.split("").any? { |letter| letter == "e" }
		end
  ...
  def test_remove_dinosaurs
    ...
    notasaurus = []
		animals.each do |animal|
			notasaurus << animal unless animal[-5..-1] == "aurus"
		end
  ...
  def test_remove_numbers
    ...
    not_numbers = []
		elements.each do |element|
			not_numbers << element unless element.is_a?(Fixnum) || element.is_a?(Float)
		end
  ...
  def test_remove_animals_starting_with_vowels
    ...
    remaining = []
		animals.each do |animal|
			remaining << animal unless /[aeiouy]/ =~ animal[0]
		end
